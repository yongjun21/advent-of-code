...
function rockFall (input, count = 2022) {
  ...
  while (count--) {
    ...
    while (true) {
      if (input[j] === '<') {
        if (rock.every(v => v % 7 !== 0) && rock.every(v => !blocked.has(v - 1))) {
          rock.forEach((v, i, arr) => {
            arr[i] = v - 1
          })
        }
      } else {
        if (rock.every(v => v % 7 !== 6) && rock.every(v => !blocked.has(v + 1))) {
          rock.forEach((v, i, arr) => {
            arr[i] = v + 1
          })
        }
        ...
        if (visited.has(state)) {
          ...
          if (cycleLength) {
            ...
            return highest +
              ...
              outcomes.slice(-cycleLength, -cycleLength + r).reduce((sum, v) => sum + v, 0)
          }
        } else {
